...
DEFAULT_SOURCE_FILES = [
    "../NPS_news_corpora_dataset/nps_elections_verified_news_seeds_2024_2026_v2.jsonl",
    "../NPS_news_corpora_dataset/nps_immigration_verified_news_seeds_2024_2026.jsonl",
    "../NPS_news_corpora_dataset/nps_public_safety_verified_news_seeds_2024_2026.jsonl",
    "../NPS_news_corpora_dataset/nps_international_conflict_verified_news_seeds_2024_2026.jsonl",
    "../NPS_news_corpora_dataset/nps_institutional_trust_verified_news_seeds_2024_2026.jsonl",
]
...
def expand_source_paths(patterns: Iterable[str | Path], base_dir: str | Path | None = None) -> list[Path]:
    """Resolve explicit JSONL paths or glob patterns in deterministic order."""
    base = Path(base_dir).resolve() if base_dir else Path.cwd().resolve()
    resolved: list[Path] = []
    for pattern in patterns:
        raw = Path(pattern)
        candidate_pattern = str(raw if raw.is_absolute() else base / raw)
        matches = sorted(Path(match).resolve() for match in glob.glob(candidate_pattern))
        if matches:
...
def main() -> None:
    ...
    parser.add_argument("--merged-output", default="NPS_Experiment/data/merged_nps_news_seeds.jsonl")
    parser.add_argument("--sampled-output", default="NPS_Experiment/data/nps_200_sampled_seeds.jsonl")
    ...
    sources = expand_source_paths(args.sources, cwd) if args.sources else _resolve_default_paths(nps_dir)
