...
def _resolve_dataset_sources(dataset_config: dict[str, Any]) -> list[Path]:
    patterns: list[str | Path] = []
    patterns.extend(dataset_config.get("source_files", []) or [])
    if dataset_config.get("source_glob"):
        patterns.append(dataset_config["source_glob"])

    if not patterns:
        return []

    source_base_dir = Path(dataset_config.get("source_base_dir", REPO_ROOT))
    if not source_base_dir.is_absolute():
        source_base_dir = REPO_ROOT / source_base_dir
    paths = expand_source_paths(patterns, source_base_dir)
    missing = [str(path) for path in paths if not path.exists()]
    if missing:
        raise FileNotFoundError(f"Configured NPS source file(s) not found: {missing}")
    return paths


def prepare_dataset(config: dict[str, Any]) -> list[dict[str, Any]]:
    dataset_config = config.get("dataset", {})
    source_files = _resolve_dataset_sources(dataset_config)
    merged_path = _resolve_repo_path(dataset_config.get("merged_path", "NPS_Experiment/data/merged_nps_news_seeds.jsonl"))
    sampled_path = _resolve_repo_path(dataset_config.get("sampled_path", "NPS_Experiment/data/nps_200_sampled_seeds.jsonl"))
...
def main() -> None:
    ...
    parser.add_argument("--config", default="NPS_Experiment/configs/experiment.yaml")
    parser.add_argument("--mode", choices=["mock", "live"], help="Override models.mode from config.")
    parser.add_argument("--dry-run", action="store_true", help="Force mock LLM mode for offline smoke tests.")
    ...
    if args.mode or args.dry_run:
        config.setdefault("models", {})["mode"] = "mock" if args.dry_run else args.mode
    ...
    trajectories_dir = ensure_dir(_resolve_repo_path(output_config.get("trajectories_dir", "NPS_Experiment/outputs/trajectories")))
    metrics_dir = ensure_dir(_resolve_repo_path(output_config.get("metrics_dir", "NPS_Experiment/outputs/metrics")))
    summaries_dir = ensure_dir(_resolve_repo_path(output_config.get("summaries_dir", "NPS_Experiment/outputs/summaries")))
